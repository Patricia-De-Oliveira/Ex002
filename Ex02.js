...
const alunos = JSON.parse(jsonTexto);

function mostrarAprovados(listaAlunos) {
    const ul = document.getElementById('aprovados');
    listaAlunos.forEach(aluno => {

        const calculo = aluno.notas.reduce((acc, nota) => acc + nota, 0);
        const media = calculo / aluno.notas.length;

        if (media >= 7) {
            const texto = `Nome: ${aluno.nome}, Curso: ${aluno.curso}, Média: ${media.toFixed(2)}`;
            console.log(texto);
            if (ul) {
